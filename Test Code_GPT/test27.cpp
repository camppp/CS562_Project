...
bool verifyGlCanvasExpectations(int num_labels, int num_major_ticks, int num_boxes) {
    if (num_labels >= num_major_ticks - 1 && num_labels <= num_major_ticks + 1) {
        if (num_boxes == num_labels + 2) {
            ...
            // Check if text rendering is within Z-layer range
            bool textInRange = mock_text_renderer.IsTextBetweenZLayers(GlCanvas::kZValueTimeBar, GlCanvas::kZValueTimeBarLabel);

            // Check if batch rendering is within Z-layer range
            bool batchInRange = mock_batcher.IsEverythingBetweenZLayers(GlCanvas::kZValueTimeBar, GlCanvas::kZValueTimeBarLabel);

            // Return true if both text and batch rendering are within Z-layer range
            return textInRange && batchInRange;
        }
    }
    return false;
}
