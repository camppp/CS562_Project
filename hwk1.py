...
Original file is located at
    https://colab.research.google.com/drive/1wOvKrMbtLhwzongyhBnpvxMBkHH9nms5
...
class UnigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        ...
        # The unigram counts are now stored in a dictionary (unigram_counts)
        self.unigram_counts = defaultdict(int)
        self.total_count = 0

        # Calculate the unigram counts for each word in the corpus
        for sentence in trainCorpus:
            for word in sentence:
                if word != '<s>': # The start-of-sentence token <s> is excluded from the unigram counts.
                    self.unigram_counts[word] += 1
                    self.total_count += 1

        # Calculate the unigram probability distribution
        self.unigram_probs = {}
        for word in self.unigram_counts:
            self.unigram_probs[word] = self.unigram_counts[word] / self.total_count
    ...
    def generateSentence(self):
        ...
        while(not sentenceEnd):
            word = random.choices(list(self.unigram_probs.keys()), list(self.unigram_probs.values()))[0]
            if word == '</s>':
                sentenceEnd = True
            elif word != '<s>':
    ...
    def getSentenceLogProbability(self, sentence):
        log_prob = 0.0
        for word in sentence:
            if word != '<s>' and word in self.unigram_probs:
                log_prob += math.log(self.unigram_probs[word], math.e) # Use natural logarithms (base e) to avoid underflow
        return log_prob
    ...
    def getCorpusPerplexity(self, testCorpus):
        ...
        for sentence in testCorpus:
            for word in sentence:
                if word != '<s>' and word in self.unigram_probs:
                    log_prob += math.log(self.unigram_probs[word], 2)
                    num_words += 1
        log_prob = log_prob / num_words # Divide by the total number of words to get the average log probability
        perplexity = 2 ** (-log_prob) # Compute perplexity as 2 to the power of the negative average log probability
        return perplexity
...
class SmoothedUnigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        # Initialize counts and other variables
        self.smoothedunigram_counts = defaultdict(int) # count of each word in the corpus
        self.total_count = 0 # total number of words in the corpus
        self.vocab_size = 0 # size of the vocabulary (number of distinct words in the corpus)

        # Count the unigrams in the corpus
        for sentence in trainCorpus:
            for word in sentence:
                if word != '<s>':
                    self.smoothedunigram_counts[word] += 1 # increment count for this word
                    self.total_count += 1 # increment total word count

        # Compute the smoothed unigram probabilities
        self.unigram_probs = {}
        self.vocab_size = len(self.smoothedunigram_counts.keys()) # get the number of distinct words in the corpus
        for word in self.smoothedunigram_counts.keys():
            # Add-one smoothing: increment count for each word, and add one to the denominator to account for unseen words
            self.unigram_probs[word] = (self.smoothedunigram_counts[word] + 1) / (self.total_count + self.vocab_size)
    ...
    def getSentenceLogProbability(self, sentence):
        ...
        for word in sentence:
            if word != '<s>' and word in self.unigram_probs:
                # Compute the log probability of the sentence as the sum of the log probabilities of each word
                log_prob += math.log(self.unigram_probs[word], math.e) # Use natural logarithms (base e) to avoid underflow
        return log_prob
...
class BigramModel(LanguageModel):
    def __init__(self, trainCorpus):
        ...
        # Count the bigrams in the corpus
        for sentence in trainCorpus:
        ...
        for word in self.vocab:
            for next_word in self.vocab:
                ...
                if self.bigram_counts[word][next_word] > 0:
                    self.bigram_probs[word][next_word] = self.bigram_counts[word][next_word] / sum(self.bigram_counts[word].values())
    ...
    def generateSentence(self):
        ...
        while True:
            ...
            word_probs = list(self.bigram_probs[prev_word].items())
            if not word_probs:
                break
    ...
    def getSentenceLogProbability(self, sentence):
        ...
        for word in sentence[1:]:
            if word in self.bigram_probs[prev_word]:
                bigram_prob = self.bigram_probs[prev_word][word]
            else:
                bigram_prob = 0.0
            log_prob += math.log(bigram_prob, 2) if bigram_prob > 0 else float('-inf')
            prev_word = word
        return log_prob
        ...
        for sentence in testCorpus:
            ...
            for word in sentence[1:]:
                if word in self.bigram_probs[prev_word]:
                    ...
                    bigram_prob = 0.0
                log_prob += math.log(bigram_prob + 1e-10, 2)
                prev_word = word
                num_words += 1
            # Add the probability of the end-of-sentence token '</s>'
            log_prob += math.log(bigram_prob + 1e-10, 2) 
            num_words += 1
        # Calculate the average log probability
        log_prob /= num_words
        # Calculate the perplexity as 2^(-log probability)
        ...
        for sentence in testCorpus:
            ...
            for word in sentence[1:]:
                if word in self.bigram_probs[prev_word]:
                    ...
                    bigram_prob = 0.0
                log_prob += math.log(1e-10, math.e)
                prev_word = word
                num_words += 1
            # Add the probability of the end-of-sentence token '</s>'
            log_prob += math.log(1e-10, math.e)
...
##### Add state
    ...
    # Rule 1
    # Rule 1a: Verbs in -ñer
   
    
    # Rule 1b: Verbs in -ñir (excluding -eñir)
    

    # Rule 2
    # Rule 2a: Verbs in -Ver
    
    # Rule 2b: Verbs in -Vir
    

    # Rule 2c*: Verbs in -guir (excluding -eguir)
    
    # Rule 2d*: Verbs in -quir
    

    # Rule 3
    # Rule 3a: Verbs in -eCir
    

    # Rule 3b: Verbs in -eCCir 


    # Rule 3c: Verbs in -eCCCir 


    # Rule 3d: Verbs in -eguir 


    # Rule 3e: Verbs in -eñir
